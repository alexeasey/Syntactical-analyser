...
public class SyntacticAnalyser {
	...
	public static ParseTree parse(List<Token> tokens) throws SyntaxException {
		...
		addChildren(parent, "prog", tokens.get(0), stack, parsingTable, tokens);

		return output;
	}

	static void addChildren(TreeNode parent, String rowTarget, Token colTarget, Deque<String> stack, String[][][] parsingTable, List<Token> tokens) throws SyntaxException {
		int num = 0;
		String stackTop = stack.peekLast();
		Token tokenTop = tokens.get(0);

		int column = findColumn(colTarget); //find terminal index
		int row = findRow(rowTarget); //find non-terminal index
		...
		for (int i = 0; i < parsingTable[row][column].length; i++) {
			...
			try { //tests if the current rule being added is terminal or non-terminal, if non-terminal it throws exception which is caught
				...
				if (s != "epsilon") { //expandable non-terminal
					TreeNode tempNode = new TreeNode(TreeNode.Label.valueOf(s), parent);
					stack.removeLast();
					parent.addChild(tempNode);
					addRule(s, tokenTop, stack, parsingTable);
					addChildren(tempNode, s, tokenTop, stack, parsingTable, tokens);
				} else { // epsilon
			...
			if (tokens.size() != 0 && stack.size() != 0) { //if one of stack or tokens = 0 but the other doesn't then wrong input
				...
				throw new SyntaxException(rowTarget + " " + row);
			}
		}
	}

	public static void addRule(String rowTarget, Token colTarget, Deque<String> stack, String[][][] parsingTable) throws SyntaxException { //Add the rules that exist in the parsing table to the stack

		int column = findColumn(colTarget);
		int row = findRow(rowTarget);
		String s = "";
		try {
	...
	public static int findColumn(Token colToken){ //find column of the terminal
		int column = colToken.getType().ordinal() + 1;
		return column;
	}

	public static int findRow(String rowToken){ //Find row of the non-terminal
		int row = TreeNode.Label.valueOf(rowToken).ordinal() + 1;
		return row;
	}
